// server.js — ChatGPT 网页源的本地 Responses 端点（取代 miuuyy 的 server.ts）。
// 监听 127.0.0.1:<port>，Bearer 校验，把 Responses/Chat 请求转成 host.runTurn 驱动网页会话。
// 回标准 OpenAI Responses SSE（tokenbank 网关按 codex-transform 同款事件消费）。
...
// 复用 tokenbank 现成的 Responses⇄Chat 转换（解析 input/messages 比手写忠实）
let transform = null;
...
const WEB_MODELS = ['chatgpt-web', 'chatgpt-web-thinking']; // P1 占位；P3 从页面能力探测

let server = null;
let state = { running: false, port: null, token: null };
...
function saveConf(c) {
  try {
    ...
    fs.writeFileSync(CONF_PATH, JSON.stringify(c, null, 2), 'utf8');
...
function flattenToPrompt(body) {
  const parts = [];
  // Responses 请求先用 tokenbank 现成 responsesToChat 归一成 messages（复用）
  let src = body;
  if (transform && Array.isArray(body.input)) {
    try { src = transform.responsesToChat(body) || body; } catch { src = body; }
  }
...
function baseResponse(respId, model, status, output) {
  return {
    ...
    status, model, output: output || [],
    usage: null, metadata: {},
  };
}

async function handleResponses(req, res, body, wantStream) {
  ...
  if (wantStream) {
    ...
    let full = '';
    try {
      const out = await host.runTurn(prompt, {
        onDelta: (delta) => {
          full += delta;
      ...
      sse(res, 'response.failed', {
        ...
        response: { ...baseResponse(respId, model, 'failed', []), error: { code: e.code || 'error', message: e.message } },
      });
      res.end(); return;
    }
    ...
    const doneItem = { id: itemId, type: 'message', role: 'assistant', status: 'completed',
  ...
  try {
    ...
    sendJson(res, e.code === 'NOT_LOGGED_IN' ? 401 : 502, { error: { code: e.code || 'error', message: e.message } });
  }
}

// ---- Chat Completions（转 Responses 内核，最小） ----
async function handleChat(req, res, body, wantStream) {
  const model = body.model || 'chatgpt-web';
  const prompt = flattenToPrompt(body);
  ...
  if (wantStream) {
    ...
    const chunk = (delta, finish) => res.write(`data: ${JSON.stringify({
      id: cid, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model,
      choices: [{ index: 0, delta, finish_reason: finish || null }],
    })}\n\n`);
    try {
      chunk({ role: 'assistant' });
      await host.runTurn(prompt, { onDelta: (d) => chunk({ content: d }) });
      chunk({}, 'stop');
    } catch (e) {
      chunk({ content: `\n[错误] ${e.message}` }, 'stop');
    }
...
function bearerOk(req) {
  ...
  if (!m || !state.token) return false;
  const a = Buffer.from(m[1]); const b = Buffer.from(state.token);
...
async function onRequest(req, res) {
  ...
  if (p === '/health' || p === '') { sendJson(res, 200, { ok: true, running: true }); return; }
  if (!bearerOk(req)) { sendJson(res, 401, { error: { message: 'invalid bearer' } }); return; }
  if (p === '/v1/models' || p === '/models') {
    sendJson(res, 200, { object: 'list', data: WEB_MODELS.map((m) => ({ id: m, object: 'model', owned_by: 'chatgpt-web' })) });
    return;
  }
  ...
  try {
    if (p === '/responses' || p === '/v1/responses') return await handleResponses(req, res, body, wantStream);
    if (p === '/chat/completions' || p === '/v1/chat/completions') return await handleChat(req, res, body, wantStream);
...
async function start() {
  ...
  const conf = loadConf();
  let port = conf.port || DEFAULT_PORT;
  const token = conf.token || id('cgw').replace('cgw_', 'cgw_');
  // 端口占用则顺延
  for (let attempt = 0; attempt < 20; attempt += 1) {
    try { server = await listenOn(port); break; }
    catch (e) { if (e.code === 'EADDRINUSE') { port += 1; continue; } throw e; }
  }
  if (!server) throw new Error('无可用端口');
  state = { running: true, port, token };
  saveConf({ port, token });
...
function stop() {
  ...
  state = { running: false, port: null, token: null };
}

function status() { return { ...state, confPath: CONF_PATH, models: WEB_MODELS }; }
